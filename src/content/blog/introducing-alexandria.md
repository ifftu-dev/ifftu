...
**A learning platform.** Courses, short-form video tutorials, and *Opinions* — credentialed takes from practitioners who already hold the skill they're commenting on. Learning is hands-on: lessons can embed interactive exercises and full in-browser code editors for Python, JavaScript, TypeScript, and C++ that run and grade your work locally — no compiler to install, no code leaving your machine, and the very engine you experiment in is the one that scores you. When you want a human, live one-on-one video tutoring — camera, audio, and screen-share — connects you to a mentor directly, peer to peer, with no call routed through a company. Every one of these lesson types is itself a sandboxed, content-addressed plugin with a frozen, permanence-oriented interface, so the community can build new kinds of lesson without waiting on me. Lessons are stored and shared peer-to-peer between users' devices (think of it as BitTorrent for coursework), and the interface already runs in nine languages — including Hindi, Bengali, Telugu, Marathi, and Urdu — because the point is access for the people currently locked out. Every new install ships with a starter catalogue, so there's something to learn on day one.
...
**Credentials you actually own.** When you demonstrate a skill — through an assessment, a project, or a peer's attestation — you earn a W3C Verifiable Credential that you sign yourself, under a `did:key` only you control. No platform holds it. Complete a course's assessments and the loop runs on its own: a Cardano validator witnesses the completion, and a local observer auto-issues the credential to you. There are six learner credential types — a formal credential, an assessment credential, a peer attestation, and so on — so a self-signed practice run reads differently from a proctored exam. Because you sign the credential yourself, anyone in the world can verify it offline, with nothing but the file you hand them, even if Alexandria itself disappears. And if you want an extra, learner-controlled layer of tamper-evidence, you can optionally anchor the credential's hash to Cardano — but the credential stands on its own signature without it.
...
**[Project Website](https://alexandria.ifftu.dev/)** — The official website for Alexandria. The alpha is going out to an early-access list first, so sign up there and I'll write when there's a build ready for your platform.
...
The core design holds up in the implementation. The reputation model, the credential pipeline, the peer-to-peer protocol, the on-device integrity layer, and the offline-first architecture are all solid enough to build on, with a large automated test suite behind them. A security audit in February surfaced 32 findings; as of the March remediation pass, 21 are fixed and 11 remain.
